...
pd.DataFrame(list(zip(sorted(glob.glob('images-to-classify/*')),*list(zip(*model.predict_classes(data_to_classify))))), columns=["image","ice"]).to_csv("classification-ice.csv",index=False)