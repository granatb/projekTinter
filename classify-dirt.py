...
model = load_model('model-dirt.h5')
...
pd.DataFrame(list(zip(sorted(glob.glob('images-to-classify/*')),*list(zip(*model.predict_classes(data_to_classify))))), columns=["image","dirt"]).to_csv("classification-dirt.csv",index=False)