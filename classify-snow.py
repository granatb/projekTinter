...
model = load_model('model-snow.h5')
...
pd.DataFrame(list(zip(sorted(glob.glob('images-to-classify/*')),*list(zip(*model.predict_classes(data_to_classify))))), columns=["image","snow"]).to_csv("classification-snow.csv",index=False)